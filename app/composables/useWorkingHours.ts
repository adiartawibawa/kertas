...
export function useWorkingHours() {
  const shifts = ref<ShiftEntry[]>([{ id: nextId(), start: '09:00', end: '17:00', breakMinutes: 60 }])

  function addShift() {
    shifts.value.push({ id: nextId(), start: '09:00', end: '17:00', breakMinutes: 60 })
  }
  ...
  const totalMinutes = computed(() => shiftMinutes.value.reduce((sum, m) => sum + m, 0))
  ...
  return { shifts, addShift, removeShift, shiftMinutes, totalMinutes, totalFormatted }
}
