...
const toolSlugs = ['pdf-merge', 'pdf-split', 'pdf-compress', 'pdf-rotate', 'pdf-extract-pages', 'pdf-to-text']

useSeoMeta({
  title: t('hubTitle.pdf'),
  description: t('categories.pdf.description'),
})
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        '@type': 'CollectionPage',
        name: t('hubTitle.pdf'),
        description: t('hubIntro.pdf'),
        hasPart: toolSlugs.map((slug) => ({
          '@type': 'SoftwareApplication',
          name: t(`tools.${slug}`),
          applicationCategory: 'UtilitiesApplication',
          url: `https://domainanda.com/office-tools/pdf/${slug}`,
        })),
      }),
    },
    {
      ...
      innerHTML: JSON.stringify({
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Office Tools', item: 'https://domainanda.com/office-tools' },
          {
            '@type': 'ListItem',
            position: 3,
            name: t('categories.pdf.name'),
            item: 'https://domainanda.com/office-tools/pdf',
          },
        ],
      }),
    },
...
<template>
  <div>
    ...
    <p class="mt-2 max-w-[56ch] text-base text-ink-soft">
      {{ t('categories.pdf.description') }}
    </p>

    <section class="mt-8">
      <p class="max-w-[68ch] text-sm text-ink-soft">{{ t('hubIntro.pdf') }}</p>
    </section>
      ...
      >
        <div>
          <h2 class="text-base font-semibold text-ink">
            <NuxtLinkLocale :to="`/office-tools/pdf/${slug}`" class="hover:text-accent-dark">
              {{ t(`tools.${slug}`) }}
            </NuxtLinkLocale>
          </h2>
