...
function downloadResult() {
  ...
  downloadBinaryFile('extracted.pdf', resultBytes.value, 'application/pdf')
}
...
<template>
  <div>
    ...
    <div v-if="totalPages > 0" class="mt-5 rounded-md border border-slate-200 bg-white p-4">
      ...
      <div class="mt-3">
        <label class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-ink-soft">{{ tp('pagesLabel')
          }}</label>
        <input v-model="pagesInput" type="text" :placeholder="tp('pagesPlaceholder')"
