...
<template>
  <div>
    ...
    <p v-if="isProcessing" class="mt-3 text-sm text-ink-soft">{{ tp('extractingLabel') }}</p>

    <div v-if="text" class="mt-5">
      <p class="mb-1.5 text-xs font-medium uppercase tracking-wide text-ink-soft">{{ tp('pagesExtractedLabel', {
        count:
          pageCount
      }) }}</p>
