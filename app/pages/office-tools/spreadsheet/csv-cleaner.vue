...
const features = computed(() =>
  (tm('toolPages.csv-cleaner.features') as { title: string; description: string }[]).map((item) => ({
    title: rt(item.title),
    description: rt(item.description),
  })),
)
const steps = computed(() => (tm('toolPages.csv-cleaner.steps') as string[]).map((item) => rt(item)))
const useCase = computed(() => (tm('toolPages.csv-cleaner.useCase') as string[]).map((item) => rt(item)))
const faqItems = computed(() =>
  (tm('toolPages.csv-cleaner.faq') as { question: string; answer: string }[]).map((item) => ({
...
useSeoMeta({
  title: () => t('toolPages.csv-cleaner.seoTitle'),
  description: () => t('toolPages.csv-cleaner.seoDescription'),
})
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        name: t('toolPages.csv-cleaner.schemaName'),
        applicationCategory: 'UtilitiesApplication',
        operatingSystem: 'Any (Web Browser)',
        offers: { '@type': 'Offer', price: '0', priceCurrency: 'USD' },
        description: t('toolPages.csv-cleaner.schemaDescription'),
      }),
    },
    {
      type: 'application/ld+json',
...
<template>
  <div>
    ...
    <h1 class="mt-3 max-w-[24ch] text-3xl font-semibold leading-tight text-ink sm:text-4xl">
      {{ t('toolPages.csv-cleaner.title') }}
    </h1>
    <p class="mt-2 max-w-[52ch] text-base text-ink-soft">
      {{ t('toolPages.csv-cleaner.lede') }}
    </p>

    <div class="mt-7">
      <FileDropzone accept=".csv,text/csv" :label="t('toolPages.csv-cleaner.dropzoneLabel')" @files="onFiles" />
    </div>

    <div class="mt-5 flex flex-wrap gap-x-6 gap-y-3 rounded-md border border-slate-200 bg-white p-4">
      <OptionToggle v-model="options.trimCells" :label="t('toolPages.csv-cleaner.optionTrimCells')" />
      <OptionToggle v-model="options.removeEmptyRows" :label="t('toolPages.csv-cleaner.optionRemoveEmptyRows')" />
      <OptionToggle v-model="options.removeDuplicateRows"
        :label="t('toolPages.csv-cleaner.optionRemoveDuplicateRows')" />
      <OptionToggle v-model="options.removeEmptyColumns" :label="t('toolPages.csv-cleaner.optionRemoveEmptyColumns')" />
    </div>

    <div v-if="cleanedCsv" class="mt-5">
      <p class="mb-1.5 text-xs font-medium uppercase tracking-wide text-ink-soft">{{
        t('toolPages.csv-cleaner.resultLabel') }}</p>
      <textarea :value="cleanedCsv" readonly rows="10"
    ...
    <div class="mt-4 flex flex-wrap items-center gap-3">
      ...
      <span v-if="rowsRemoved > 0" class="text-sm text-ink-soft">{{ t('toolPages.csv-cleaner.rowsRemovedLabel', {
        count:
          rowsRemoved }) }}</span>
    </div>
    ...
    <section class="mt-14">
      <h2 class="text-xl font-semibold text-ink">{{ t('toolPages.csv-cleaner.featuresTitle') }}</h2>
    ...
    <section class="mt-14">
      <h2 class="text-xl font-semibold text-ink">{{ t('toolPages.csv-cleaner.howToTitle') }}</h2>
    ...
    <section class="mt-14">
      <h2 class="text-xl font-semibold text-ink">{{ t('toolPages.csv-cleaner.useCaseTitle') }}</h2>
